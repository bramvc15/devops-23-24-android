package com.example.templateapplication.shared

import com.example.templateapplication.models.Doctor
import java.time.YearMonth


fun generateDoctors(): List<Doctor> = buildList {

    val currentMonth = YearMonth.now()

    currentMonth.atDay(26).also { date ->
        add(
            Doctor(
                1,
                "Dr. J. Van der Veken",
                 "Male",
                 "Eye Specialist",
                "Dr. J. Van der Veken is een ervaren oogarts met een passie voor het verbeteren van het gezichtsvermogen van zijn patiënten. Met jarenlange ervaring in de oogheelkunde, is hij toegewijd aan het bieden van hoogwaardige oogzorg.",
                "Dr. Van der Veken voltooide zijn medische opleiding aan de Universiteit van Amsterdam en behaalde zijn specialisatiediploma in oogheelkunde aan de Erasmus Universiteit Rotterdam. Hij heeft expertise in de behandeling van diverse oogaandoeningen.",
                 "Dr. Van der Veken heeft bijgedragen aan oogheelkundig onderzoek en heeft diverse onderscheidingen ontvangen voor zijn toewijding aan oogzorg.",
                "https://images.healthshots.com/healthshots/en/uploads/2022/07/02195043/doctor-stress.jpg",
                "123"
            ),
        )
        add(
            Doctor(
                2,
                "Dr. G. De Craene",
                "Male",
                "Eye Specialist",
                "Dr. J. Van der Veken is een ervaren oogarts met een passie voor het verbeteren van het gezichtsvermogen van zijn patiënten. Met jarenlange ervaring in de oogheelkunde, is hij toegewijd aan het bieden van hoogwaardige oogzorg.",
                "Dr. Van der Veken voltooide zijn medische opleiding aan de Universiteit van Amsterdam en behaalde zijn specialisatiediploma in oogheelkunde aan de Erasmus Universiteit Rotterdam. Hij heeft expertise in de behandeling van diverse oogaandoeningen.",
                "Dr. Van der Veken heeft bijgedragen aan oogheelkundig onderzoek en heeft diverse onderscheidingen ontvangen voor zijn toewijding aan oogzorg.",
                "https://images.healthshots.com/healthshots/en/uploads/2022/07/02195043/doctor-stress.jpg",
                "321"
                ),
        )
    }

}
